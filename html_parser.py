...
# if os.path.exists():
#     shutil.rmtree(mirai_root_folder + citation_folder + '/' + year)

for type in parse_list:

    html_file_name = f'MIRAI_{type}.html'
    ...
    for html_tag in html_soup.findAll('div', {'class': 'csl-entry'}):
        ...
        if not os.path.exists(mirai_root_folder + citation_folder + '/' + year):
            os.makedirs(mirai_root_folder + citation_folder + '/' + year)
        with open(mirai_root_folder + citation_folder + '/' + year + '/' + file_name, 'w') as f:
